...
class BoundPropertySpec_Validation : FixtureSteppable
{
    ...
    ObservableProperty<string> Property { get; } = new(string.Empty);

    public BoundPropertySpec_Validation()
    {
        PropertyNotAttributedValidation = string.Empty.ToBoundProperty().Bind(Property);
        PropertyAttributedValidation = string.Empty.ToBoundProperty().Bind(Property);
        PropertyAttributedMultiValidations = string.Empty.ToBoundProperty().Bind(Property);
        RequiredProperty = string.Empty.ToBoundProperty().Bind(Property);
        LocalizablePropertyAttributedValidation = string.Empty.ToBoundProperty().Bind(Property);

        ErrorsChangedVerificationHandler = (_, _) => ErrorsChangedRaised = true;
    }
