...
public abstract class BindableProperty<T> : INotifyPropertyChanged, IDataErrorInfo, INotifyDataErrorInfo
{
    ...
    /// </summary>
    protected List<BindingSourceContext> BindingSources { get; } = new();

    /// <summary>
    /// Gets the state of the validation for the property value.
    /// </summary>
    protected virtual ValidationState Validation { get; } = new();
    ...
    public BindableProperty<T> EnableValidation(Expression<Func<BindableProperty<T>>> selector)
        => EnableValidation(selector, false);
    ...
    public BindableProperty<T> EnableValidation(Expression<Func<BindableProperty<T>>> selector, bool cancelValueChangedIfInvalid)
    {
        ...
        if (selector.Body is not MemberExpression memberExpression) throw new ArgumentException("The body of the selector must be MemberExpression.");
        if (memberExpression.Member is not PropertyInfo property) throw new ArgumentException("The member of the body of the selector must be PropertyInfo.");
    ...
    public BindableProperty<T> Bind<TSource>(BindableProperty<TSource> source, Func<TSource, T> converter)
    {
        ...
        {
            if (s is not BindableProperty<TSource> sourceProperty) return;
    ...
    public BindableProperty<T> Bind(Func<MultiBindingContext, T> converter, params INotifyPropertyChanged[] sources)
    {
        ...
        BindingSources.AddRange(sources.Select(source => new BindingSourceContext(source, (_, e) =>
        {
            if (e.PropertyName != ValuePropertyName) return;
    ...
    {
        return Validator.TryValidateValue(propertyValue!, new ValidationContext(this) { MemberName = assignedPropertyName, DisplayName = displayName ?? string.Empty }, results, validations);
    }

    private PropertyChangedEventArgs EnsureValueChangedEventArgs() => valueChangedEventArgs ??= new PropertyChangedEventArgs(ValuePropertyName);
    private DataErrorsChangedEventArgs EnsureDataErrorsChangedEventArgs() => dataErrorsChangedEventArgs ??= new DataErrorsChangedEventArgs(ValuePropertyName);
...
{
    public static readonly PropertyChangedEventArgs HasErrorsChangedEventArgs = new(nameof(BindableProperty<object?>.HasErrors));
    public static readonly PropertyChangedEventArgs ErrorChangedEventArgs = new(nameof(BindableProperty<object?>.Error));
    public static readonly PropertyChangedEventArgs ErrorsChangedEventArgs = new(nameof(BindableProperty<object?>.Errors));
}